...
public class verifyHimc extends BaseTest {
...
@Test(dataProvider = "testcase", dataProviderClass = Dataproviders.class)
   @ExcelData(sheetName ="Sheet12",rowIndex= {1,2})
   	...
   	((JavascriptExecutor) driver).executeScript("arguments[0].click();", searchButton);
	       ...
	       try {
	          ...
	          // driver.findElement(By.id("Go")).click();
	           WebElement acctID = wait.until(ExpectedConditions.presenceOfElementLocated(By.id("invtTranId")));
	           String AcctID = input.getByHeader("Tran_Id");
	           ((JavascriptExecutor) driver).executeScript("arguments[0].value=arguments[1];", acctID, AcctID);
	           driver.findElement(By.id("Go")).click();
	           
	    	 //  WindowHandle.setValueWithJS(driver, wait.until(ExpectedConditions.presenceOfElementLocated(By.id("invtTranId"))),input.getByHeader("Tran_Id"));
