...
public class TC13_ModifyAnExistingGuarantee {

    private WebDriver driver;
    private WebDriverWait wait;
    private static final String EXCEL_PATH = System.getProperty("user.dir") + "/Resource/FBTCDATA.xlsx";
    private static final String SHEET_NAME = "TC13_ModifyAnExistingGuarantee";
    ...
    public void runTest() throws Exception {
        String logFile = TestResultLogger.createLogFile("TC13");
        Login login = new Login();

        try {
            // 🔹 Load Test Data
            RowData initialData = ExcelUtils.getRowAsRowData("TC04", 1); // Initial row
            RowData modifyData = ExcelUtils.getRowAsRowData(SHEET_NAME, 1); // Row 1 = Modify
            RowData verifyData = ExcelUtils.getRowAsRowData(SHEET_NAME, 2); // Row 2 = Verify

            // ---------------- Step 1: Login as Maker ----------------
            login.First();
            System.out.println("🟡 Starting Modification for Guarantee...");
            OutwardGuaranteeMaintenance guarantee = new OutwardGuaranteeMaintenance(driver);

            // ---------------- Step 2: Modify Guarantee ----------------
            Map<String, String> modifyResult = guarantee.executeWithResultMap(modifyData, initialData, SHEET_NAME, 1, EXCEL_PATH);
            String guaranteeNo = modifyResult.get("GuaranteeNo");
           // String transactionId = modifyResult.get("TransactionID");
            String labelText = modifyResult.get("LabelText");

            assertGuaranteeCaptured(guaranteeNo, logFile, "Modify");
            TestResultLogger.log(logFile, labelText);

            // ---------------- Step 3: Logout Maker ----------------
            LogOut.performLogout(driver, wait);
            System.out.println("✅ Logout performed successfully after Modify.");
            ...
            //String verifyTransactionId = verifyResult.get("TransactionID");
